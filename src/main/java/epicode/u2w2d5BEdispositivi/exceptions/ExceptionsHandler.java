...
import epicode.u2w2d5BEdispositivi.payload.errors.ErrorsPayloadWithList;
...
public class ExceptionsHandler {
    ...
    public ErrorsPayloadWithList handleBadRequest(BadRequestException ex) {
        ...
        if(ex.getErrorsList() != null) {
            errorsMessage = ex.getErrorsList().stream().map(err -> err.getDefaultMessage()).toList();
        }
        return new ErrorsPayloadWithList(ex.getMessage(), new Date(), errorsMessage);
    }
